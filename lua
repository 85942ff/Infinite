...
local Tabs = {
    ...
    SilentAim = Window:AddTab('静默自瞄', 'crosshair'),
    Visuals = Window:AddTab("静默fov", "camera"),
    Management = Window:AddTab("黑名单", "users"),
    Misc = Window:AddTab("静默杂项", "box"),
...
-- ========== 定义 Toggles 和 Options 表 ==========
local Toggles = {}
local Options = {}

-- ========== 静默自瞄主设置 ==========
local MainSettingsBox = Tabs.SilentAim:AddLeftGroupbox("主设置")

local enabledToggle = MainSettingsBox:AddToggle("EnabledToggle", { Text = "启用", Default = SilentAimSettings.Enabled })
enabledToggle:AddKeyPicker("EnabledKeybind", { Default = SilentAimSettings.ToggleKey, SyncToggleState = true, Mode = "Toggle" })
enabledToggle:OnChanged(function(Value) SilentAimSettings.Enabled = Value end)
Toggles.EnabledToggle = enabledToggle

local teamCheckToggle = MainSettingsBox:AddToggle("TeamCheckToggle", { Text = "队伍检查", Default = SilentAimSettings.TeamCheck })
teamCheckToggle:OnChanged(function(Value) SilentAimSettings.TeamCheck = Value end)
Toggles.TeamCheckToggle = teamCheckToggle

local visibleCheckToggle = MainSettingsBox:AddToggle("VisibleCheckToggle", { Text = "可见性检查", Default = SilentAimSettings.VisibleCheck })
visibleCheckToggle:OnChanged(function(Value) SilentAimSettings.VisibleCheck = Value end)
Toggles.VisibleCheckToggle = visibleCheckToggle

local wallbangToggle = MainSettingsBox:AddToggle("WallbangToggle", { Text = "穿墙", Default = SilentAimSettings.Wallbang })
wallbangToggle:OnChanged(function(Value) SilentAimSettings.Wallbang = Value end)
Toggles.WallbangToggle = wallbangToggle

local leakHitToggle = MainSettingsBox:AddToggle("LeakAndHitToggle", { Text = "漏打模式", Default = SilentAimSettings.LeakAndHitMode })
leakHitToggle:OnChanged(function(Value) SilentAimSettings.LeakAndHitMode = Value end)
Toggles.LeakAndHitToggle = leakHitToggle

local hitChanceSlider = MainSettingsBox:AddSlider('HitChanceSlider', { Text = '命中率', Default = SilentAimSettings.HitChance, Min = 0, Max = 100, Rounding = 1, Suffix = "%" })
hitChanceSlider:OnChanged(function(Value) SilentAimSettings.HitChance = Value end)
Options.HitChanceSlider = hitChanceSlider

-- ========== 目标 ==========
local TargetingBox = Tabs.SilentAim:AddRightGroupbox("目标")

local targetModeDropdown = TargetingBox:AddDropdown("TargetModeDropdown", { Text = "目标种类", Default = "请选择", Values = {"玩家", "NPC", "所有"} })
targetModeDropdown:OnChanged(function(Value) SilentAimSettings.TargetMode = Value end)
Options.TargetModeDropdown = targetModeDropdown

local targetPartDropdown = TargetingBox:AddDropdown("TargetPartDropdown", { Values = {"Head", "HumanoidRootPart", "Random"}, Default = SilentAimSettings.TargetPart, Text = "目标部位" })
targetPartDropdown:OnChanged(function(Value) SilentAimSettings.TargetPart = Value end)
Options.TargetPartDropdown = targetPartDropdown

local priorityModeDropdown = TargetingBox:AddDropdown("PriorityModeDropdown", { Text = "优先模式", Default = SilentAimSettings.PriorityMode, Values = {"准星最近", "距离最近", "最低血量", "最近的人(无FOV)"} })
priorityModeDropdown:OnChanged(function(Value) SilentAimSettings.PriorityMode = Value end)
Options.PriorityModeDropdown = priorityModeDropdown

local maxDistanceSlider = TargetingBox:AddSlider('MaxDistanceSlider', { Text = '最大距离', Default = SilentAimSettings.MaxDistance, Min = 10, Max = 2000, Rounding = 0, Suffix = "studs" })
maxDistanceSlider:OnChanged(function(Value) SilentAimSettings.MaxDistance = Value end)
Options.MaxDistanceSlider = maxDistanceSlider

-- ========== 方法 ==========
local MethodBox = Tabs.SilentAim:AddRightGroupbox("方法")

local methodDropdown = MethodBox:AddDropdown("MethodDropdown", { Text = "静默瞄准方式", Default = SilentAimSettings.SilentAimMethod, Values = { "Raycast","FindPartOnRay", "FindPartOnRayWithWhitelist", "FindPartOnRayWithIgnoreList", "ScreenPointToRay", "ViewportPointToRay", "Ray", "Mouse.Hit/Target" } })
methodDropdown:OnChanged(function(Value) SilentAimSettings.SilentAimMethod = Value end)
Options.MethodDropdown = methodDropdown

local predictionToggle = MethodBox:AddToggle("PredictionToggle", { Text = "Mouse.Hit/Target 预判", Default = SilentAimSettings.MouseHitPrediction })
predictionToggle:OnChanged(function(Value) SilentAimSettings.MouseHitPrediction = Value end)
Toggles.PredictionToggle = predictionToggle

local predictionAmountSlider = MethodBox:AddSlider("PredictionAmountSlider", { Text = "预判量", Min = 0, Max = 1, Default = SilentAimSettings.MouseHitPredictionAmount, Rounding = 3 })
predictionAmountSlider:OnChanged(function(Value) SilentAimSettings.MouseHitPredictionAmount = Value; PredictionAmount = Value end)
Options.PredictionAmountSlider = predictionAmountSlider

local headshotChanceToggle = MethodBox:AddToggle("HeadshotChanceToggle", { Text = "启用爆头几率", Default = SilentAimSettings.HeadshotChanceEnabled })
headshotChanceToggle:OnChanged(function(Value) SilentAimSettings.HeadshotChanceEnabled = Value end)
Toggles.HeadshotChanceToggle = headshotChanceToggle

local headshotChanceSlider = MethodBox:AddSlider('HeadshotChanceSlider', { Text = '爆头概率', Default = SilentAimSettings.HeadshotChance, Min = 0, Max = 100, Rounding = 1, Suffix = "%" })
headshotChanceSlider:OnChanged(function(Value) SilentAimSettings.HeadshotChance = Value end)
Options.HeadshotChanceSlider = headshotChanceSlider

-- ========== 范围与指示器 ==========
local FovIndicatorBox = Tabs.Visuals:AddLeftGroupbox("范围与指示器")

local fovVisibleToggle = FovIndicatorBox:AddToggle("FOVVisibleToggle", { Text = "显示FOV圈", Default = SilentAimSettings.FOVVisible })
local fovColorPicker = fovVisibleToggle:AddColorPicker("FOVColorPicker", { Default = Color3.fromRGB(54, 57, 241), Title = "FOV圈颜色" })
fovVisibleToggle:OnChanged(function(Value) FOVCircleGui.Enabled = Value; SilentAimSettings.FOVVisible = Value end)
fovColorPicker:OnChanged(function(Value) 
    if type(Value) == "Color3" then FOVStroke.Color = Value end
end)
Toggles.FOVVisibleToggle = fovVisibleToggle
Options.FOVColorPicker = fovColorPicker

local fovRadiusSlider = FovIndicatorBox:AddSlider("FOVRadiusSlider", { Text = "FOV圈半径", Min = 10, Max = 1000, Default = SilentAimSettings.FOVRadius, Rounding = 0 })
fovRadiusSlider:OnChanged(function(Value) FOVCircleFrame.Size = UDim2.fromOffset(Value * 2, Value * 2); SilentAimSettings.FOVRadius = Value end)
Options.FOVRadiusSlider = fovRadiusSlider

local fixedFOVToggle = FovIndicatorBox:AddToggle("FixedFOVToggle", { Text = "固定FOV (移动端)", Default = SilentAimSettings.FixedFOV })
fixedFOVToggle:OnChanged(function(Value) SilentAimSettings.FixedFOV = Value end)
Toggles.FixedFOVToggle = fixedFOVToggle

local showTargetToggle = FovIndicatorBox:AddToggle("ShowTargetToggle", { Text = "显示目标", Default = SilentAimSettings.ShowSilentAimTarget })
local targetIndicatorColorPicker = showTargetToggle:AddColorPicker("TargetIndicatorColorPicker", { Default = Color3.fromRGB(255,0,0), Title = "指示器颜色" })
showTargetToggle:OnChanged(function(Value) SilentAimSettings.ShowSilentAimTarget = Value end)
targetIndicatorColorPicker:OnChanged(function(Value)
    if type(Value) == "Color3" then
        target_indicator_circle.Color = Value
        for _, line in ipairs(target_indicator_lines) do line.Color = Value end
    end
end)
Toggles.ShowTargetToggle = showTargetToggle
Options.TargetIndicatorColorPicker = targetIndicatorColorPicker

local indicatorStyleDropdown = FovIndicatorBox:AddDropdown("IndicatorStyleDropdown", { Text = "指示器样式", Values = {"Circle", "Triangle", "Pentagram", "十字准星", "三线准星"}, Default = "Circle" })
Options.IndicatorStyleDropdown = indicatorStyleDropdown

local targetIndicatorRadiusSlider = FovIndicatorBox:AddSlider("TargetIndicatorRadiusSlider", { Text = "指示器大小(通用)", Min = 5, Max = 50, Default = SilentAimSettings.TargetIndicatorRadius, Rounding = 0 })
targetIndicatorRadiusSlider:OnChanged(function(Value) SilentAimSettings.TargetIndicatorRadius = Value end)
Options.TargetIndicatorRadiusSlider = targetIndicatorRadiusSlider

local crosshairLengthSlider = FovIndicatorBox:AddSlider("CrosshairLengthSlider", { Text = "十字准星长度", Min = 5, Max = 100, Default = SilentAimSettings.CrosshairLength, Rounding = 0 })
crosshairLengthSlider:OnChanged(function(Value) SilentAimSettings.CrosshairLength = Value end)
Options.CrosshairLengthSlider = crosshairLengthSlider

local crosshairGapSlider = FovIndicatorBox:AddSlider("CrosshairGapSlider", { Text = "十字准星间隙", Min = 0, Max = 50, Default = SilentAimSettings.CrosshairGap, Rounding = 0 })
crosshairGapSlider:OnChanged(function(Value) SilentAimSettings.CrosshairGap = Value end)
Options.CrosshairGapSlider = crosshairGapSlider

local indicatorRotationToggle = FovIndicatorBox:AddToggle("IndicatorRotationToggle", { Text = "指示器旋转", Default = SilentAimSettings.IndicatorRotationEnabled })
indicatorRotationToggle:OnChanged(function(Value) SilentAimSettings.IndicatorRotationEnabled = Value end)
Toggles.IndicatorRotationToggle = indicatorRotationToggle

local indicatorRotationSpeedSlider = FovIndicatorBox:AddSlider("IndicatorRotationSpeedSlider", { Text = "旋转速度", Min = 0, Max = 10, Default = SilentAimSettings.IndicatorRotationSpeed, Rounding = 1 })
indicatorRotationSpeedSlider:OnChanged(function(Value) SilentAimSettings.IndicatorRotationSpeed = Value end)
Options.IndicatorRotationSpeedSlider = indicatorRotationSpeedSlider

local indicatorRainbowToggle = FovIndicatorBox:AddToggle("IndicatorRainbowToggle", { Text = "启用彩虹色", Default = SilentAimSettings.IndicatorRainbowEnabled })
indicatorRainbowToggle:OnChanged(function(Value) SilentAimSettings.IndicatorRainbowEnabled = Value end)
Toggles.IndicatorRainbowToggle = indicatorRainbowToggle

local indicatorRainbowSpeedSlider = FovIndicatorBox:AddSlider("IndicatorRainbowSpeedSlider", { Text = "颜色变换速度", Min = 0, Max = 10, Default = SilentAimSettings.IndicatorRainbowSpeed, Rounding = 1 })
indicatorRainbowSpeedSlider:OnChanged(function(Value) SilentAimSettings.IndicatorRainbowSpeed = Value end)
Options.IndicatorRainbowSpeedSlider = indicatorRainbowSpeedSlider

local indicatorBreathingToggle = FovIndicatorBox:AddToggle("IndicatorBreathingToggle", { Text = "启用呼吸效果", Default = SilentAimSettings.IndicatorBreathingEnabled })
indicatorBreathingToggle:OnChanged(function(Value) SilentAimSettings.IndicatorBreathingEnabled = Value end)
Toggles.IndicatorBreathingToggle = indicatorBreathingToggle

local indicatorBreathingSpeedSlider = FovIndicatorBox:AddSlider("IndicatorBreathingSpeedSlider", { Text = "呼吸速度", Min = 0.1, Max = 5, Default = SilentAimSettings.IndicatorBreathingSpeed, Rounding = 1 })
indicatorBreathingSpeedSlider:OnChanged(function(Value) SilentAimSettings.IndicatorBreathingSpeed = Value end)
Options.IndicatorBreathingSpeedSlider = indicatorBreathingSpeedSlider

local indicatorBreathingMinSlider = FovIndicatorBox:AddSlider("IndicatorBreathingMinSlider", { Text = "呼吸最小比例", Min = 0.1, Max = 1, Default = SilentAimSettings.IndicatorBreathingMin, Rounding = 2 })
indicatorBreathingMinSlider:OnChanged(function(Value) SilentAimSettings.IndicatorBreathingMin = Value end)
Options.IndicatorBreathingMinSlider = indicatorBreathingMinSlider

local indicatorBreathingMaxSlider = FovIndicatorBox:AddSlider("IndicatorBreathingMaxSlider", { Text = "呼吸最大比例", Min = 1, Max = 3, Default = SilentAimSettings.IndicatorBreathingMax, Rounding = 2 })
indicatorBreathingMaxSlider:OnChanged(function(Value) SilentAimSettings.IndicatorBreathingMax = Value end)
Options.IndicatorBreathingMaxSlider = indicatorBreathingMaxSlider

local threeLineCrosshairToggle = FovIndicatorBox:AddToggle("ThreeLineCrosshairToggle", { Text = "启用三线准星", Default = SilentAimSettings.ThreeLineCrosshairEnabled })
threeLineCrosshairToggle:OnChanged(function(Value) SilentAimSettings.ThreeLineCrosshairEnabled = Value end)
Toggles.ThreeLineCrosshairToggle = threeLineCrosshairToggle

local threeLineCrosshairLengthSlider = FovIndicatorBox:AddSlider("ThreeLineCrosshairLengthSlider", { Text = "三线准星长度", Min = 5, Max = 100, Default = SilentAimSettings.ThreeLineCrosshairLength, Rounding = 0 })
threeLineCrosshairLengthSlider:OnChanged(function(Value) SilentAimSettings.ThreeLineCrosshairLength = Value end)
Options.ThreeLineCrosshairLengthSlider = threeLineCrosshairLengthSlider

local threeLineCrosshairGapSlider = FovIndicatorBox:AddSlider("ThreeLineCrosshairGapSlider", { Text = "三线准星间隙", Min = 0, Max = 50, Default = SilentAimSettings.ThreeLineCrosshairGap, Rounding = 0 })
threeLineCrosshairGapSlider:OnChanged(function(Value) SilentAimSettings.ThreeLineCrosshairGap = Value end)
Options.ThreeLineCrosshairGapSlider = threeLineCrosshairGapSlider

-- ========== 信息 ==========
local InfoBox = Tabs.Visuals:AddRightGroupbox("信息")

local targetInfoStyleDropdown = InfoBox:AddDropdown("TargetInfoStyleDropdown", { Text = "信息显示样式", Default = SilentAimSettings.TargetInfoStyle, Values = {"面板", "头顶", "独立面板"} })
targetInfoStyleDropdown:OnChanged(function(Value) SilentAimSettings.TargetInfoStyle = Value end)
Options.TargetInfoStyleDropdown = targetInfoStyleDropdown

local showTargetNameToggle = InfoBox:AddToggle("ShowTargetNameToggle", { Text = "显示目标名字", Default = SilentAimSettings.ShowTargetName })
showTargetNameToggle:OnChanged(function(Value) SilentAimSettings.ShowTargetName = Value end)
Toggles.ShowTargetNameToggle = showTargetNameToggle

local showTargetHealthToggle = InfoBox:AddToggle("ShowTargetHealthToggle", { Text = "显示目标血量", Default = SilentAimSettings.ShowTargetHealth })
showTargetHealthToggle:OnChanged(function(Value) SilentAimSettings.ShowTargetHealth = Value end)
Toggles.ShowTargetHealthToggle = showTargetHealthToggle

local showTargetDistanceToggle = InfoBox:AddToggle("ShowTargetDistanceToggle", { Text = "显示目标距离", Default = SilentAimSettings.ShowTargetDistance })
showTargetDistanceToggle:OnChanged(function(Value) SilentAimSettings.ShowTargetDistance = Value end)
Toggles.ShowTargetDistanceToggle = showTargetDistanceToggle

local showTargetCategoryToggle = InfoBox:AddToggle("ShowTargetCategoryToggle", { Text = "显示目标类别", Default = SilentAimSettings.ShowTargetCategory })
showTargetCategoryToggle:OnChanged(function(Value) SilentAimSettings.ShowTargetCategory = Value end)
Toggles.ShowTargetCategoryToggle = showTargetCategoryToggle
...
local pinPanelToggle = InfoBox:AddToggle("PinPanelToggle", {Text = "固定面板", Default = SilentAimSettings.IndependentPanelPinned})
pinPanelToggle:OnChanged(function(value)
    SilentAimSettings.IndependentPanelPinned = value
    IndependentPanelFrame.Draggable = not value
end)
Toggles.PinPanelToggle = pinPanelToggle

-- ========== 额外 ==========
local ExtrasBox = Tabs.Visuals:AddRightGroupbox("额外")

local highlightToggle = ExtrasBox:AddToggle("HighlightToggle", { Text = "启用高亮", Default = SilentAimSettings.HighlightEnabled })
local highlightColorPicker = highlightToggle:AddColorPicker("HighlightColorPicker", { Default = SilentAimSettings.HighlightColor, Title = "高亮颜色" })
highlightToggle:OnChanged(function(Value) SilentAimSettings.HighlightEnabled = Value end)
highlightColorPicker:OnChanged(function(Value) 
    if type(Value) == "Color3" then SilentAimSettings.HighlightColor = Value end
end)
Toggles.HighlightToggle = highlightToggle
Options.HighlightColorPicker = highlightColorPicker

local highlightRainbowToggle = ExtrasBox:AddToggle("HighlightRainbowToggle", { Text = "高亮彩虹色", Default = SilentAimSettings.HighlightRainbowEnabled })
highlightRainbowToggle:OnChanged(function(Value) SilentAimSettings.HighlightRainbowEnabled = Value end)
Toggles.HighlightRainbowToggle = highlightRainbowToggle

local damageNotifierToggle = ExtrasBox:AddToggle("DamageNotifierToggle", { Text = "显示伤害通知", Default = SilentAimSettings.ShowDamageNotifier })
damageNotifierToggle:OnChanged(function(Value) SilentAimSettings.ShowDamageNotifier = Value end)
Toggles.DamageNotifierToggle = damageNotifierToggle

local hitSoundDropdown = ExtrasBox:AddDropdown('HitSound', { Text = '击中音效', Default = '关闭', Values = {'关闭', 'bell', 'metal', 'click', 'exp'} })
Options.HitSound = hitSoundDropdown

local showTracerToggle = ExtrasBox:AddToggle("ShowTracerToggle", { Text = "显示目标追踪线", Default = SilentAimSettings.ShowTracer })
local tracerColorPicker = showTracerToggle:AddColorPicker("TracerColorPicker", { Default = tracer_line.Color, Title = "追踪线颜色" })
showTracerToggle:OnChanged(function(Value) SilentAimSettings.ShowTracer = Value end)
tracerColorPicker:OnChanged(function(Value)
    if type(Value) == "Color3" then tracer_line.Color = Value end
end)
Toggles.ShowTracerToggle = showTracerToggle
Options.TracerColorPicker = tracerColorPicker

local tracerYOffsetSlider = ExtrasBox:AddSlider('TracerYOffsetSlider', { Text = '追踪线Y轴偏移', Default = SilentAimSettings.Tracer_Y_Offset, Min = -10, Max = 10, Rounding = 3, Suffix = " studs" })
tracerYOffsetSlider:OnChanged(function(Value) SilentAimSettings.Tracer_Y_Offset = Value end)
Options.TracerYOffsetSlider = tracerYOffsetSlider

-- ========== 管理 - 手动锁定 ==========
local ManualLockGroupBox = Tabs.Management:AddLeftGroupbox("手动锁定")

local targetSelectorDropdown = ManualLockGroupBox:AddDropdown("TargetSelectorDropdown", { Text = "锁定目标 (无=自动)", Default = "无", Values = {"无"} })
targetSelectorDropdown:OnChanged(function(selectedName)
...
Options.TargetSelectorDropdown = targetSelectorDropdown
...
ManualLockGroupBox:AddButton("刷新列表", function()
    ...
    targetSelectorDropdown:SetValues(targetNames, "无")
    lockedTargetObject = nil
end)
...
local enableNameTargetingToggle = NameTargetingGroup:AddToggle("EnableNameTargetingToggle", { Text = "启用名称索敌", Default = SilentAimSettings.EnableNameTargeting })
enableNameTargetingToggle:OnChanged(function(Value)
    SilentAimSettings.EnableNameTargeting = Value
end)
Toggles.EnableNameTargetingToggle = enableNameTargetingToggle
...
local whitelistNameInput = NameTargetingGroup:AddInput("WhitelistNameInput", { Text = "名称", PlaceholderText = "输入要锁定的NPC名称关键字" })
Options.WhitelistNameInput = whitelistNameInput

NameTargetingGroup:AddButton("添加到列表", function()
    local name = whitelistNameInput.Value
    if name and name ~= "" then
        table.insert(SilentAimSettings.WhitelistedNames, name)
        whitelistDropdown:SetValues(SilentAimSettings.WhitelistedNames)
        whitelistNameInput:SetValue("")
        updateWhitelistData()
    end
end)

local whitelistDropdown = NameTargetingGroup:AddDropdown("WhitelistDropdown", { Text = "名称列表", Values = SilentAimSettings.WhitelistedNames or {} })
Options.WhitelistDropdown = whitelistDropdown

NameTargetingGroup:AddButton("从列表中删除", function()
    local selectedName = whitelistDropdown.Value
...
local whitelistPathInput = WhitelistPathGroup:AddInput("WhitelistPathInput", { Text = "路径", PlaceholderText = "输入从Workspace开始的路径" })
Options.WhitelistPathInput = whitelistPathInput

WhitelistPathGroup:AddButton("添加路径", function()
    local path = whitelistPathInput.Value
    if path and path ~= "" then
        table.insert(SilentAimSettings.WhitelistPath, path)
        whitelistPathDropdown:SetValues(SilentAimSettings.WhitelistPath)
        whitelistPathInput:SetValue("")
    end
end)

local whitelistPathDropdown = WhitelistPathGroup:AddDropdown("WhitelistPathDropdown", { Text = "路径列表", Values = SilentAimSettings.WhitelistPath or {} })
Options.WhitelistPathDropdown = whitelistPathDropdown

WhitelistPathGroup:AddButton("删除路径", function()
    local selectedPath = whitelistPathDropdown.Value
...
local blacklistNameInput = BlacklistGroup:AddInput("BlacklistNameInput", { Text = "名称", PlaceholderText = "输入要拉黑的精确名称" })
Options.BlacklistNameInput = blacklistNameInput

BlacklistGroup:AddButton("添加到黑名单", function()
    local name = blacklistNameInput.Value
    if name and name ~= "" and not isBlacklisted(name) then
        table.insert(SilentAimSettings.BlacklistedNames, name)
        blacklistDropdown:SetValues(SilentAimSettings.BlacklistedNames)
        blacklistNameInput:SetValue("")
        updateBlacklistData()
    end
end)

local blacklistDropdown = BlacklistGroup:AddDropdown("BlacklistDropdown", { Text = "黑名单列表", Values = SilentAimSettings.BlacklistedNames or {} })
Options.BlacklistDropdown = blacklistDropdown

BlacklistGroup:AddButton("从黑名单中删除", function()
    local selectedName = blacklistDropdown.Value
    if selectedName then
...
local function transparencyLoop()
    ...
    local isRainbowEnabled = Toggles.TransparentCharacterRainbow and Toggles.TransparentCharacterRainbow.Value
...
local transparentCharToggle = CharacterModGroup:AddToggle("TransparentCharacterEnabled", { Text = "人物透明", Default = false })
transparentCharToggle:OnChanged(function(value)
    if value then
...
Toggles.TransparentCharacterEnabled = transparentCharToggle

local transparentRainbowToggle = CharacterModGroup:AddToggle("TransparentCharacterRainbow", { Text = "人物变色", Default = false })
transparentRainbowToggle:OnChanged(function(value)
...
Toggles.TransparentCharacterRainbow = transparentRainbowToggle
...
local spinToggle = EntertainmentGroup:AddToggle("SpinToggle", { Text = "启用旋转", Default = false })
...
Toggles.SpinToggle = spinToggle

local spinSpeedSlider = EntertainmentGroup:AddSlider("SpinSpeedSlider", { Text = "旋转速度", Default = 10, Min = 1, Max = 100, Rounding = 0 })
spinSpeedSlider:OnChanged(function(value)
    spinSpeed = math.rad(value)
end)
Options.SpinSpeedSlider = spinSpeedSlider

-- ========== 初始化 FOV 等 ==========
FOVCircleGui.Enabled = Toggles.FOVVisibleToggle and Toggles.FOVVisibleToggle.Value
if Options.FOVColorPicker and type(Options.FOVColorPicker.Value) == "Color3" then
    FOVStroke.Color = Options.FOVColorPicker.Value
end
if Options.FOVRadiusSlider and Options.FOVRadiusSlider.Value then
    ...
    RenderStepped:Connect(function()
            ...
            if humanoid then
                ...
                if lastHealth and currentHealth < lastHealth then
                    ...
                    if damage > 0 then
                        ...
                        local selectedSoundName = Options.HitSound and Options.HitSound.Value
                        if selectedSoundName and selectedSoundName ~= '关闭' then
        ...
        if isEnabled and currentTargetPart then
            ...
            if IsOnScreen and Toggles.ShowTargetToggle and Toggles.ShowTargetToggle.Value then
                ...
                local indicatorStyle = Options.IndicatorStyleDropdown and Options.IndicatorStyleDropdown.Value or "Circle"
                ...
                elseif SilentAimSettings.IndicatorRainbowEnabled then finalIndicatorColor = Color3.fromHSV(currentIndicatorHue, 1, 1)
                    ...
                    local pickerColor = Options.TargetIndicatorColorPicker and Options.TargetIndicatorColorPicker.Value
                    finalIndicatorColor = (type(pickerColor) == "Color3" and pickerColor) or Color3.fromRGB(255,0,0)
                end
            ...
            local showAnyInfo = (Toggles.ShowTargetNameToggle and Toggles.ShowTargetNameToggle.Value) or (Toggles.ShowTargetHealthToggle and Toggles.ShowTargetHealthToggle.Value) or (Toggles.ShowTargetDistanceToggle and Toggles.ShowTargetDistanceToggle.Value) or (Toggles.ShowTargetCategoryToggle and Toggles.ShowTargetCategoryToggle.Value)
            if showAnyInfo then
                ...
                if humanoid and localRoot then
                    ...
                    if infoStyle == "独立面板" then
                        ...
                        independent_panel_texts.Name.Visible = Toggles.ShowTargetNameToggle and Toggles.ShowTargetNameToggle.Value
                        independent_panel_texts.Health.Visible = Toggles.ShowTargetHealthToggle and Toggles.ShowTargetHealthToggle.Value
                        independent_panel_texts.Distance.Visible = Toggles.ShowTargetDistanceToggle and Toggles.ShowTargetDistanceToggle.Value
                        independent_panel_texts.Category.Visible = Toggles.ShowTargetCategoryToggle and Toggles.ShowTargetCategoryToggle.Value
                        if Toggles.ShowTargetNameToggle and Toggles.ShowTargetNameToggle.Value then independent_panel_texts.Name.Text = "目标: " .. targetName end
                        if Toggles.ShowTargetHealthToggle and Toggles.ShowTargetHealthToggle.Value then independent_panel_texts.Health.Text = string.format("血量: %d", health) end
                        if Toggles.ShowTargetDistanceToggle and Toggles.ShowTargetDistanceToggle.Value then independent_panel_texts.Distance.Text = string.format("距离: %dm", dist) end
                        if Toggles.ShowTargetCategoryToggle and Toggles.ShowTargetCategoryToggle.Value then independent_panel_texts.Category.Text = "类别: " .. category end
                    elseif infoStyle == "面板" and IsOnScreen then
                        ...
                        if Toggles.ShowTargetNameToggle and Toggles.ShowTargetNameToggle.Value then local textObj = panel_info_texts.Name; textObj.Text = targetName; textObj.Position = infoPos + Vector2.new(5, 5 + (linesDrawn * lineHeight)); textObj.Visible = true; linesDrawn = linesDrawn + 1 end
                        if Toggles.ShowTargetHealthToggle and Toggles.ShowTargetHealthToggle.Value then local textObj = panel_info_texts.Health; textObj.Text = string.format("血量: %d", health); textObj.Position = infoPos + Vector2.new(5, 5 + (linesDrawn * lineHeight)); textObj.Visible = true; linesDrawn = linesDrawn + 1 end
                        if Toggles.ShowTargetDistanceToggle and Toggles.ShowTargetDistanceToggle.Value then local textObj = panel_info_texts.Distance; textObj.Text = string.format("距离: %dm", dist); textObj.Position = infoPos + Vector2.new(5, 5 + (linesDrawn * lineHeight)); textObj.Visible = true; linesDrawn = linesDrawn + 1 end
                        if Toggles.ShowTargetCategoryToggle and Toggles.ShowTargetCategoryToggle.Value then local textObj = panel_info_texts.Category; textObj.Text = "类别: " .. category; textObj.Position = infoPos + Vector2.new(5, 5 + (linesDrawn * lineHeight)); textObj.Visible = true; linesDrawn = linesDrawn + 1 end
                    ...
                    elseif infoStyle == "头顶" and IsOnScreen then
                        ...
                        if Toggles.ShowTargetNameToggle and Toggles.ShowTargetNameToggle.Value then local textObj = overhead_info_texts.Name; textObj.Text = string.format("[%s]", targetName); textObj.Position = Vector2.new(RootToViewportPoint.X, base_y - (linesDrawn * lineHeight)); textObj.Visible = true; linesDrawn = linesDrawn + 1 end
                        if Toggles.ShowTargetHealthToggle and Toggles.ShowTargetHealthToggle.Value then local textObj = overhead_info_texts.Health; textObj.Text = string.format("[%d]", health); textObj.Position = Vector2.new(RootToViewportPoint.X, base_y - (linesDrawn * lineHeight)); textObj.Visible = true; linesDrawn = linesDrawn + 1 end
                        if Toggles.ShowTargetDistanceToggle and Toggles.ShowTargetDistanceToggle.Value then local textObj = overhead_info_texts.Distance; textObj.Text = string.format("[%dm]", dist); textObj.Position = Vector2.new(RootToViewportPoint.X, base_y - (linesDrawn * lineHeight)); textObj.Visible = true; linesDrawn = linesDrawn + 1 end
                        if Toggles.ShowTargetCategoryToggle and Toggles.ShowTargetCategoryToggle.Value then local textObj = overhead_info_texts.Category; textObj.Text = string.format("[%s]", category); textObj.Position = Vector2.new(RootToViewportPoint.X, base_y - (linesDrawn * lineHeight)); textObj.Visible = true; linesDrawn = linesDrawn + 1 end
                    end
                end
            end
        ...
        if Toggles.ShowTracerToggle and Toggles.ShowTracerToggle.Value and isEnabled and currentTargetPart then
            ...
            if IsOnScreen then
                ...
                local tracerColor = Options.TracerColorPicker and Options.TracerColorPicker.Value
                tracer_line.Color = (type(tracerColor) == "Color3" and tracerColor) or Color3.fromRGB(255,255,0)
            end
        else
        ...
        if Toggles.FOVVisibleToggle and Toggles.FOVVisibleToggle.Value then
            if Toggles.FixedFOVToggle and Toggles.FixedFOVToggle.Value then FOVCircleFrame.Position = UDim2.fromScale(0.5, 0.5) else local mousePos = GetMouseLocation(UserInputService); FOVCircleFrame.Position = UDim2.fromOffset(mousePos.X, mousePos.Y) end
        end
    end)
end))
